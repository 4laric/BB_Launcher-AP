...
class ApUiTest final : public QObject {
  ...
  private slots:
    ...
    void seedChoicePlayReturnAndSwitch() {
        ...
        auto* preserve = page.findChild<QCheckBox*>(QStringLiteral("apEnemyPreserveLocomotion"));
        auto* scaling = page.findChild<QCheckBox*>(QStringLiteral("apEnemyNormalizeStats"));
        auto* bosses = page.findChild<QCheckBox*>(QStringLiteral("apShuffleBosses"));
        QVERIFY(seedEdit && player && play && randomize && changeSeed);
        QVERIFY(enemyMode && advanced && enemySeed && preserve && scaling && bosses);
        ...
        QVERIFY(preserve->isVisible() && preserve->isEnabled());
        QVERIFY(scaling->isVisible() && scaling->isEnabled());
        QVERIFY(bosses->isVisible() && bosses->isEnabled());
        QTest::mouseClick(preserve, Qt::LeftButton);
        QTest::mouseClick(scaling, Qt::LeftButton);
        QVERIFY(preserve->isChecked());
        QVERIFY(scaling->isChecked());
        QTest::mouseClick(bosses, Qt::LeftButton);
        QVERIFY(!bosses->isChecked());
        QTest::mouseClick(bosses, Qt::LeftButton);
        QVERIFY(bosses->isChecked());
